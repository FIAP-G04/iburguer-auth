...
WORKDIR /src
COPY ["IBurguer.Onboarding.Disable.csproj", "IBurguer.Onboarding.Disable/"]
RUN dotnet restore "IBurguer.Onboarding.Disable/IBurguer.Onboarding.Disable.csproj"

WORKDIR "/src/IBurguer.Onboarding.Disable"
COPY . .
RUN dotnet build "IBurguer.Onboarding.Disable.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "IBurguer.Onboarding.Disable.csproj" \
...
Execute unit tests
```
cd "IBurguer.Onboarding.Disable/test/IBurguer.Onboarding.Disable.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "IBurguer.Onboarding.Disable/src/IBurguer.Onboarding.Disable"
